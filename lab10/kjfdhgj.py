...
class Gun(Agent):
    ...
    def update(self):
        ...
        if self.f2_on:
            self.f2_power = min(
                self.f2_power + self.gun_power_gain, self.max_gun_power)#FUCK
        else:
    ...
    def get_gunpoint(self):
        length = self.f2_power + self.zero_power_length #FUCK
...
class BattleField(tk.Canvas):
    def __init__(self, master):
        ...
        self.gun = Gun(self)
        self.gun2 = Gun(self)
        self.gun2.gun_coords = [550,450]#FUCK возможно придётся поменять сам coords
        self.targets = {}
        self.bullets = {}
        self.get_root().bind("<Control-u>", self.change_gun)#changes here
        self.change_gun_flag = True #changes here
    ...
    def change_gun(self, event):
        self.change_gun_flag = True if self.change_gun_flag==False else False
        self.check_flag()
    def check_flag(self):
        print(self.change_gun_flag)
        if self.change_gun_flag:
            self.gun.pause()
            self.gun2.start()
            self.gun2.play()
        else:
...
class GunGameApp(tk.Tk):
    def __init__(self):
       ...
       # self.battlefield = BattleField(self)#changes
        ...
        #self.battlefield.bind("<Control-u>", self.change_gun)#changes here
